...
class BaseTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        ffn: nn.Module,
        norm: nn.Module,
        time_step_embed : nn.Module = None,
        time_step_embed_query:nn.Module = None,
        ...
        self.time_step_embed = time_step_embed
        self.time_step_embed_query = time_step_embed_query
        ...
        if time_step_embed_query is not None:
            ...
            self.time_step_embed_query = None
        # self.time_step_embed = time_step_embed
        # self.time_step_embed = time_step_embed
    ...
    def forward(
        ...
        **kwargs contains the specific arguments of attentions.
        ...
        for layer in self.operation_order:
            if layer == "self_attn":
                if t is not None:
                    # query_class , query_denoising = query.chunk(2 , dim = -1)
                    # query = self.time_step_embeds[t_index](query_denoising, t)
                    # query  = torch.cat([query_class , query], dim = -1)

                    # query = self.time_step_embeds[t_index](query, t)
                    if "no_queries" in kwargs:
                        x = query[:,kwargs['no_queries'][0]:,:]
                        x = self.time_step_embeds[t_index](x, t)
                        y = query[:,:kwargs['no_queries'][0],:]
                        query = torch.cat([y, x], 1)
                    else:
                        query = self.time_step_embeds[t_index](query, t)
                    t_index += 1
                ...
                #     kwargs['reference_points'] = self.time_step_embed(reference_points , t)
                if t is not None:
                    # query_class , query_denoising = query.chunk(2 , dim = -1)
                    # query = self.time_step_embeds[t_index](query_denoising, t)
                    # query  = torch.cat([query_class , query], dim = -1)
                    ...
                    # query = self.time_step_embeds[t_index](query, t)
                    if "no_queries" in kwargs:
                        # query[:,kwargs['no_queries'][0]:,:] = self.time_step_embeds[t_index](query[:,kwargs['no_queries'][0]:,:], t)
                        x = query[:,kwargs['no_queries'][0]:,:]
                        x = self.time_step_embeds[t_index](x, t)
                        y = query[:,:kwargs['no_queries'][0],:]
                        query = torch.cat([y, x], 1)
            ...
            # if t is not None:
            #     query = self.time_step_embeds[1](query , t)
            elif layer == "ffn":
                if t is not None:
                    # query_class , query_denoising = query.chunk(2 , dim = -1)
                    # query = self.time_step_embeds[t_index](query_denoising, t)
